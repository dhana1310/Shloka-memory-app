import { Box, Button, Card, FormControl, InputLabel, LinearProgress, MenuItem, Paper, Select, Switch, Typography } from "@mui/material";
...
const DisplayGita = () => {
  ...
  const ParaDisplay = ({ line }) => {
    if (
      ...
      return <p style={{ color: "#1b5e20", fontWeight: "bold", fontSize: "35px" }}>{line}</p>;
    }
    return <p style={{ fontWeight: "bold", fontSize: "25px" }}>{line}</p>;
  };
